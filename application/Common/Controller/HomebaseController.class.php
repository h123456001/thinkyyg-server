	...
	function _initialize() {
		...
		if($ucenter_syn){
			if(!isset($_SESSION["user"])){
				if(!empty($_COOKIE['ThinkYYG_auth'])  && $_COOKIE['ThinkYYG_auth']!="logout"){
					$ThinkYYG_auth=sp_authcode($_COOKIE['ThinkYYG_auth'],"DECODE");
					$ThinkYYG_auth=explode("\t", $ThinkYYG_auth);
					$auth_username=$ThinkYYG_auth[1];
					$users_model=M('Users');
