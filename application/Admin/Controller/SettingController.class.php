...
class SettingController extends AdminbaseController{
	...
	function site(){
		...
		$YYG_settings=$this->options_model->where("option_name='YYG_settings'")->getField("option_value");
		...
		$this->assign("YYG_settings",json_decode($YYG_settings,true));
	...
	function site_post(){
		if (IS_POST) {
			...
			$banned_usernames=preg_replace("/[^0-9A-Za-z_\x{4e00}-\x{9fa5}-]/u", ",", $_POST['YYG_settings']['banned_usernames']);
			$_POST['YYG_settings']['banned_usernames']=$banned_usernames;

			sp_set_YYG_setting($_POST['YYG_settings']);
